...
; If config.ini is missing, these defaults will be used:
...
; PauseKey defaults to "Del" based on your request (Terminate)
...
RegisterKey(KeyName, WeaponName) {
    ...
    try {
        ...
        MsgBox("Could not register key: " KeyName)
    }
}
...
TogglePause(*) {
    ...
    Paused := !Paused
    if Paused
        ToolTip("PAUSED (Script Disabled)")
    else
        ToolTip("RESUMED")
    SetTimer(() => ToolTip(), -1500)
}
